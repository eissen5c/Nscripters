		...
		else
		{
				if ($data->email == $email)
				{
					...
					{ 
							...
							echo '<h1 class="mbr-hero__text">CHANGE PASSWORD</h1>';
							echo '<form method="POST">';
							echo '<input class="form-control" type="password" id="oldpassword"  name="oldpassword" placeholder="Old Password" required="" /><br/>';	
							echo '<input class="form-control" type="password" id="password"  name="password" placeholder="New Password" required="" /><br/>';	
							echo '<input class="form-control" type="password" id="repassword" name="repassword" placeholder="Re Password" required="" /><br/>';
							...
							if(isset($_POST['CHANGE']))
							{
								...
								if ($data->password == MD5($old))
								{
									if ($new == $renew)
									{
										...
										$renew2 = MD5($renew);
										$query="UPDATE userpanganiban set `password` ='{$renew2}' WHERE email='{$email}'";
										$result = $db_connection->query($query);
										
										$_SESSION['emaill'] = $email;
										$_SESSION['passwordd'] = $renew;

										echo '<h1 class="mbr-hero__text">SUCCESS</h1>';
										
									}
									else
									{
									echo '<h1 class="mbr-hero__text">PASSWORD DOES NOT MATCH!</h1>';
									}
								}
							}
