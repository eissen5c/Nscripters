...
function ifEmpty()
{
	...
	else
	{
		...
		ajaxFunction('./sendchat.php?chatinput='+document.getElementById("chatinput").value,'textarea');
		eraseText();
	}
}
		ajaxFunction("./recievechat.php","textarea");
		
		var refInterval = window.setInterval('ajaxFunction("./recievechat.php","textarea")', 3000); 
  </script>
					<h1>CHAT (testing)</h1>
					<div id="chatborder">
					welcome <?php echo $_SESSION['firstt'];?>
						<div id="chatcon">
							<div id="textarea" class="editable" contenteditable="false">
							<h1>LOADING</h1>
							</div>
							<div id="chatusers">
							<?php print_r($_SESSION['firstt']); ?>
							</div>
						</div>
						<form id="submitt" name="sendchat" autocomplete="off" >
						<!--<input id="chatinput" type="text" name="chatinput" placeholder="Message here"/>-->
						<textarea require="" maxlength="255" id="chatinput" name="chatinput" placeholder="Message here" style="margin: 7.5px;
    width: 69%;
    height: 59px;
    resize: none;
"></textarea>
						<input id="chatsubmit" type="button" name="chatsubmit"  onclick="ifEmpty();" value="Send"/>
